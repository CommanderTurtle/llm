...
Each Soft action has a persisted, per-session Auto choice. The first thrown token/context-capacity error in one generation may apply the selected fresh Soft groups and re-enter the existing Continue path once. Other failures, a second capacity error, and a capacity error with nothing new to compact remain terminal for that run.
...
Streaming output coalesces browser paints and mutates only stable content/reasoning/tool nodes inside the active assistant article rather than reconstructing the card, transcript, or session list per token. Expensive rich highlighting waits until the message is terminal. Context totals are cached per session; while the terminal assistant turn grows, the meter measures only appended text instead of serializing the full projected history twice per second. Closed context-read and indexed-resource disclosures render their Markdown on first open. Stable streaming scroll retains the reasoning disclosure and its independent scroll position in place, and follows output only while the reader is near the bottom. With the box clear, the original forced-follow behavior remains.
